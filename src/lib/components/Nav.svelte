<script lang="ts">
    ...
    onMount(() => {
        ...
        buttons.forEach(button => observer.observe(document.getElementById(button.target) as HTMLElement));
    });
</script>
...
<nav id="nav" class="flex flex-row justify-center p-5 w-screen text-regular fixed top-0 left-0 right-0 z-50 transition-all">
    <div>
        {#each buttons as button (button.target)}
            <NavButton {...button} isActive={activeButton === button.target} on:click={() => clickHandler(button)} />
        {/each}
    </div>
</nav>
