...
<div class="flex flex-col justify-center p-4 border-solid bg-crust shadow-lg rounded-lg [&>*]:my-2 {isLink ? "group transition-all duration-700 hover:scale-[1.02]" : ""} {$$props.class}">
    <div>
        ...
        <div class="flex flex-row justify-between self-between items-center">
            {#if tags.length}
                <ul class="list-none flex flex-row items-center text-center text-crust text-sm">
                    {#each tags as tag}
                        <li class="bg-blue rounded-full py-1 px-2 mx-1">{tag.toUpperCase()}</li>
                    {/each}
                </ul>
            {/if}
            {#if isLink}
                <div class="flex flex-row justify-center items-center">
                    <p class="text-sm text-subtext1 italic max-md:hidden lg:visible group-hover:text-blue transition-colors">view source</p>
                    <Icon icon="ep:arrow-right" width={22} height={22} class="group-hover:text-blue transition-colors" />
                </div>
            {/if}
        </div>
    {/if}
